  ...
  const triggerAISteward = async (userMessage: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        model: GEMINI_MODEL,
        contents: `You are the AI Steward for a nomad community formation group in ${location.name}, ${location.city}, ${location.country}. 
        The community focus is: ${location.intellectualTags.join(', ')}.
        Description: ${location.description}.
        
        A user just said: "${userMessage}". 
        Provide a helpful, concise response (max 2 sentences) guiding them on community formation, 
        resource pledging, or local highlights specific to this location. 
        Act as a consultant for community acquisition and a guide for this specific place.
        Be professional yet welcoming.`,
      });

      const aiResponse = response.text || "I'm here to help you navigate the community formation process in " + location.name + ".";
    ...
    <div className="flex flex-col h-full bg-stone-900 border border-white/5 overflow-hidden">
      <div className="p-3 border-b border-white/5 bg-stone-800/50 flex justify-between items-center">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[8px] font-bold text-emerald-500 uppercase tracking-tighter">AI Steward Active</span>
        </div>
      </div>
      ...
      >
        {messages.length === 0 && !isTyping && (
          <div className="flex flex-col items-center justify-center h-full py-4 text-center">
            <Bot size={24} className="text-emerald-500/40 mb-2" />
        ...
        {messages.map((msg) => (
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              {msg.isAI && <Bot size={10} className="text-emerald-500" />}
              {!msg.isAI && <User size={10} className="text-stone-500" />}
              <span className={`text-[9px] font-bold uppercase tracking-wider ${msg.isAI ? 'text-emerald-500' : 'text-stone-500'}`}>
                {msg.senderName}
              </span>
            </div>
            <div className={`px-3 py-2 rounded-xl max-w-[90%] text-[11px] leading-relaxed ${
              msg.senderId === currentUser?.uid 
                ? 'bg-emerald-500 text-stone-950 font-medium rounded-tr-none' 
                : msg.isAI
                  ? 'bg-emerald-500/10 text-emerald-100 rounded-tl-none border border-emerald-500/20'
        ...
        {isTyping && (
          <div className="flex flex-col items-start">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-500">AI Steward</span>
            </div>
            <div className="bg-emerald-500/5 border border-emerald-500/10 px-3 py-2 rounded-xl rounded-tl-none">
      ...
      {currentUser ? (
        <form onSubmit={handleSendMessage} className="p-3 border-t border-white/5 flex gap-2 bg-stone-900">
          ...
          <button 
            ...
            className="w-8 h-8 rounded-xl bg-emerald-500 text-stone-950 flex items-center justify-center hover:bg-emerald-400 transition-colors shadow-lg disabled:opacity-40"
          >
            <Send size={14} />
          </button>
        </form>
